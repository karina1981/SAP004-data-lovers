...
describe('sortData', () => {
  ...
  it('apply sorted name ascendente', () => {
    ...
    expect(sortData[0].name).toBe('Blastoise');
    expect(sortData[sortData.length-1].name).toBe('Weedle');
  });

  it('apply sorted name descendente', () => {
    let sortData = search.sortData(pokemons.pokemon, "descendente");
    expect(sortData[0].name).toBe('Weedle');
    expect(sortData[sortData.length-1].name).toBe('Blastoise');
  });
});
...
describe('computeStats', () => {
  ...
  it('calculate type percentages', () => {
    expect(search.computeStats(pokemons.pokemon, 'Grass')).toBe('21.43')
    expect(search.computeStats(pokemons.pokemon, 'Bug')).toBe('35.71')
  })
});
